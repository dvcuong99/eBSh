...
{
    public class TopCareController : Controller
    {
        ...
        public TopCareController(ITopCarePremiumRepo premiumRepo, IPA_GCNRepository pa_GCNRepo, IGiftCodeRepository giftCodeRepo, IOrderRepository orderRepo)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (regInfo.CODE_KM != null && regInfo.CODE_KM.Trim().Length > 0)
                    {
                        ...
                        var gcInf = GiftCodeRepo.GetByCode(regInfo.CODE_KM, "TOPCARE", regInfo.CTBH);
                        ...
                        else
                        {
                            ...
                            regInfo.NSD = gcInf.Ma_NSD; ;// người duyệt đơn
                            if (gcInf.CTBH == null)
                                gcInf.CTBH = "";
                            if (gcInf.MTN == null)
                                gcInf.MTN = "";
                    ...
                    if (regInfo.TL_KM >= 0 && regInfo.TL_KM < 100)
                    {
                        ...
                        if (so_id > 0)
                        {
                            ...
                            if (regInfo.TT == "T")
                            {
                                ...
                                orderInf = new OrderInfo { PolicyID = so_id, Amount = PhiTT, CreatedDate = DateTime.Now, OrderDescription = "Thanh toan don BH TopCare - Ma KH:" + regInfo.CMND + " - Ten:" + regInfo.KhachHang + " - HLBH:" + regInfo.Ngay_HL, PolicyPreviewUrl = strUrl };
                    ...
                    else// đơn tặng 100% phí
                    {
                        ...
                        if (so_id > 0)
                        {
                            ...
                            if (regInfo.TT == "T")
                            {
                                ...
                                var strUrl = await nvcoreSvc.PA_LinkPreGCNAPI(duyetData);
                                orderInf = new OrderInfo { PolicyID = so_id, Amount = regInfo.Phi_BH, CreatedDate = DateTime.Now, OrderDescription = "Don BH TopCare tang 100% phi - KH:" + regInfo.CMND + regInfo.KhachHang, PolicyPreviewUrl = strUrl };
                                var vm = new PAOrderVM { paInf = regInfo, orderInf = orderInf };
                                //Chuyển màn hình xác nhận thanh toán
        ...
        public ActionResult acceptOrder(PAOrderVM vm)
        {
            ...
            try
            {
                ...
                if (lastOrder.OrderId > 0)
                {
                    ...
                    {
                        var duyetData = new GCNVM { Ma_Dvi = vm.paInf.Ma_Dvi, So_ID = lastOrder.PolicyID, CB_Du = vm.paInf.NSD };
                        var so_hd = AsyncUtil.RunSync<string>(() => nvcoreSvc.PA_DuyetAPI(duyetData));
                ...
                vnpInf.vnpUrlReturn = ConfigurationManager.AppSettings["vnp_Returnurl"] + "/topcare/vnpayresult"; //URL nhan ket qua tra ve 
        ...
        public ActionResult acceptOrderKM(PAOrderVM vm)
        {
            ...
            try
            {
                ...
                {
                    var gcInf = GiftCodeRepo.GetByCode(cbInf.CODE_KM, "TOPCARE", cbInf.CTBH);
                ...
                if (vnpayData.Count > 0)
                {
                    ...
                    if (checkSignature)
                    {
                        if (vnp_ResponseCode == "00")
                        {
                            ...
                            if (lastOrder.OrderId > 0)
                            {
                                if (lastOrder.Amount == amount)
                                {
                                    ...
                                    CoreNVService nvcoreSvc = CoreNVService.getInstance();
                                    var duyetData = new GCNVM { Ma_Dvi = cbInf.Ma_Dvi, So_ID = lastOrder.PolicyID, CB_Du = cbInf.NSD };
                                    var so_hd = AsyncUtil.RunSync<string>(() => nvcoreSvc.PA_DuyetAPI(duyetData));
                        ...
                        else
                        {
                            ...
                            //ViewBag.NGTT = "Ngày: " + lastOrder.vnp_ResponseDate.ToString("dd/MM/yyyy");
                            ViewBag.Success = "";
                            Log.Error("Thanh toan loi, OrderId={0}, BSH TopCare TranId={1},ResponseCode={2}, Description ={3}", orderId, vnpayTranId, vnp_ResponseCode, errDes);
                        }
                    }
                    else
                    {
