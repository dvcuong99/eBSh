...
{
    public class MVCManageController : Controller
    {
        IMotorVehicleRepository motorVehicleRepository;
        public MVCManageController(IMotorVehicleRepository motorVehicleRepository)
        {
        ...
        public ActionResult Index(string SDate, string EDate, string MaDvi = "all", string Kenh = "all", string DaiLy = "all", int Page = 1)
        {
            ...
            var data = motorVehicleRepository.GetList(_SDate, _EDate, _maDvi.ToUpper(), _maKenh.ToUpper(), _maDaiLy.ToUpper());
            var mvcCardPage = new MVCPaginatedVM
            {
            ...
            return View(mvcCardPage);
        }

        public ActionResult Detail(decimal id)
        {
            MotorVehicle mvcInf = new MotorVehicle();
            if (id != 0)
            {
                try
                {
                    mvcInf = motorVehicleRepository.GetByID(id);
                    return View(mvcInf);
                }
                catch (Exception)
                {
        ...
        public ActionResult ExportData(string MA_CTBH, string SDate, string EDate, string MaDvi, string Kenh, string DaiLy)
        {
            ...
            var data = motorVehicleRepository.GetList(_SDate, _EDate, MaDvi, Kenh, DaiLy);
            dt = Helper.Common.ToDataTable<MotorVehicle>(data.ToList());
